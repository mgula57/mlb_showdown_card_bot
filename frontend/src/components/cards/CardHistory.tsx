...
import { FaCircleCheck, FaCircleXmark, FaShuffle, FaImage } from "react-icons/fa6";
...
export const CardHistory = ({ history, onSelectCard }: CardHistoryProps) => {
    ...
    const getImageUrl = (record: CustomCardLogRecord): string | null => {
        ...
        // Priority 1: Thumbnail (small, optimized for list view)
        if (record.thumbnail_storage_path) {
            return `${supabaseUrl}/storage/v1/object/public/card_images/${record.thumbnail_storage_path}`;
        }

        // Priority 2: Full storage path (larger, for logged-in users)
        if (record.storage_path) {
            return `${supabaseUrl}/storage/v1/object/public/card_images/${record.storage_path}`;
        }

        // Priority 3: Other image URLs
                ...
                {!history || history.length === 0 ? (
                    ...
                    <>
                        {groupedArray.map((group, index) => (
                            <li key={index}>
                                <div className="flex gap-2 py-2 rounded-lg bg-(--background-primary) px-3">

                                    {/* Thumbnail */}
                                    <div className="shrink-0 w-16 rounded overflow-hidden bg-(--background-secondary)" style={{ aspectRatio: '2.5 / 3.5' }}>
                                        {getImageUrl(group.record) ? (
                                            <img
                                                src={getImageUrl(group.record)!}
