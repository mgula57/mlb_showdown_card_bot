...
def fetch_war(type:str) -> list[dict]:
    return fetch_table_data(url=f'https://www.baseball-reference.com/leaders/WAR_{type}_season.shtml', table_id=f"leader_standard_WAR_{type}", bref_id_field_substring='players/', use_header_for_col_names=True)
...
war_dicts = fetch_war(type='bat')
war_dicts_sorted = sorted(war_dicts, key=lambda d: float(d.get('WAR Position Players', 0)), reverse=True)
for player_dict in war_dicts_sorted:
    try:
        ...
        if bref_id not in bref_ids_w_image and (bref_id in hof_bref_ids if args.hof else True):
            print(player_dict['Player (age that year)'], player_dict['Year'], player_dict['WAR Position Players'])
    except:
        continue
