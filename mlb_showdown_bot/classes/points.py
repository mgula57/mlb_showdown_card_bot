...
class Points(BaseModel):
    ...
    def total_points_unrounded(self):
        return ((self.ba + self.obp + self.slg + self.hr + self.defense + self.speed + self.ip + self.icons + self.position_adjustment + self.command) * self.ip_multiplier ) + self.out_distribution
    
    @property
    def breakdown_str(self) -> str:
        """Strings with list of all points categories"""
        
        breakdown_categories = ['ba','obp','slg','hr','defense','speed','ip','icons','out_distribution','position_adjustment','command',]
        other_attributes = ['command_out_multiplier', 'ip_multiplier', 'decay_rate', 'decay_start',]
