...
class ShowdownTeamBuilder:
    """Compose a read-only team builder Team from a pool of card summary rows.

    Used to present real MLB/WBC rosters (sourced from the card archive) in the
    team builder UI. Unlike autofill (budget-driven and randomized), composition
    is deterministic and based on real playing time: lineup spots go to the
    eligible hitters with the most games played, the rotation is ordered by games
