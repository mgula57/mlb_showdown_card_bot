...
from ..utils.shared_functions import convert_to_numeric, fill_empty_stat_categories

class BaseballReferenceScraper(BaseModel):
    ...
    disable_cleaning_cache: bool = False
    cache_folder_path: str = None
    disable_stats_period_range_updates: bool = False
    is_running_on_website: bool = False

    # GENERATED BY RUNNING STATS
    is_name_a_bref_id: bool = False
    source: str = None
    cache_folder_path: str = os.path.join(Path(os.path.dirname(__file__)),'cache_data')
        ...
        if self.team_override is None:
            for team_id in [team.value for team in Team]:
                ...
                if team_match and not self.stats_period.is_multi_year and not self.stats_period.is_full_career and self.stats_period.split is None:
                    self.team_override = Team(team_id)
...
# ------------------------------------------------------------------------
    ...
    def fetch_player_stats(self) -> dict:
        """Fetch player data from baseball reference.
        
        Accounts for querying cache or acrhive data first.
        Order of operations:
          1. Local Cache
          2. Archive
          3. Scrape
        ...
        Returns:
          Player data dictionary with stats and metadata.
        """
        ...
        if not self.ignore_cache:
            ...
            if cached_data:
                self.stats_period.source = 'Local Cache'
...
# SCRAPE WEBSITES
    ...
    def __check_for_default_bref_id(self, name:str, years: list[str]) -> str:
        ...
        # LOAD CSV
        player_id_filepath = os.path.join(Path(os.path.dirname(__file__)).parent,'data','player_ids.csv')
...
# ------------------------------------------------------------------------
    ...
    def scrape_baseball_reference(self) -> dict:
        ...
        years_for_loop = self.stats_period.year_list
        ...
        overall_type = self.type(positions_dict=positions_and_games, year=self.year)
        ...
        for year in years_for_loop:
            ...
            if not has_ran_oaa and is_hitter: # ONLY NEEDS TO RUN ONCE FOR MULTI-YEAR
                years_list = years_played if self.stats_period.is_full_career else self.stats_period.year_list
        ...
        # SAVE DATA        
        self.stats_period.source = f"Baseball Reference{'/Baseball Savant' if is_data_from_statcast else ''}"
    ...
    def type(self, positions_dict:dict[str,dict], year:int) -> str:
        ...
        if total_games == 0:
            if self.stats_period.is_multi_year:
                ...
                if not (self.stats_period.is_multi_year or self.stats_period.is_full_career):
