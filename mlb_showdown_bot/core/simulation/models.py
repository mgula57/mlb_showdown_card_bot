...
class SeasonSimulationConfig(BaseModel):
    ...
    min_ip_sp: int = 50
    min_ip_rp: int = 30
    # WHEN ENABLED, ANY CARD BELOW THE FULL-SAMPLE PA REFERENCE (600) IS REBUILT WITH ITS RATE
    # STATS REGRESSED TOWARD THAT YEAR'S REPLACEMENT LEVEL BEFORE ROSTER TIERING/POINTS-SORTING -
    # SEE `Roster._regressed_to_replacement_if_small_sample`. KEEPS A CALLUP'S OR SPOT STARTER'S
    # HOT SMALL SAMPLE FROM OUTVALUING A PROVEN REGULAR'S FULL SEASON PURELY ON NOISE. REAL
    # PA/GS/IP (WHAT TIERING GATES ON) ARE UNCHANGED EITHER WAY.
    regress_small_sample_stats: bool = False
    ...
    # POSTSEASON-ONLY RESUME. WHEN TRUE, NO REGULAR-SEASON GAMES ARE SIMULATED AT ALL - EVERY
    # CLUB'S RECORD IS SEEDED FROM THE REAL FINAL STANDINGS (THE API LAYER ALWAYS SETS
    # `resume_from_real_season` ALONGSIDE THIS FOR THAT REASON), AND THE POSTSEASON BRACKET ITSELF
    # IS SEEDED FROM REAL RESULTS PLAYED SO FAR - SEE `RealPostseasonBracket` / `Postseason.simulate`.
    # ONLY VALID FOR A TAKEOVER-FREE RUN: A TAKEOVER CLUB REPLACES A REAL CLUB'S ROSTER, SO SEEDING
    # IN THAT CLUB'S ACTUAL POSTSEASON RESULTS WOULD HAND IT SERIES WINS IT NEVER PLAYED.
    # `Season.simulate` RAISES IF BOTH `is_takeover` AND THIS ARE SET.
    resume_from_real_postseason: bool = False
